...
- **失敗的影片會被無限重試**。變更 2 的 catch 不記錄失敗次數、不推遲下次撿取。
  ...
  對目前已知的問題是多餘的機制。
- **加界的兩條查詢不做公平輪替，持續失敗的文件會排擠同一條查詢的其他候選**。
  - 顧慮：`_id` 不可變，所以固定排序加上限在「有一批文件持續 save 失敗」的前提
    下必然餓死另一端——降序會讓較舊的候選永遠進不了那 25 個名額，升序則反過來
    讓較新的影片進不去。方向只決定誰被排擠，不消除排擠本身。
  - 決定：保留 `.sort({ _id: -1 }).limit(25)`，不加 rotating cursor、不加 retry
    backoff。
  - 理由：這個情境要成立，得先有一批文件持續 save 失敗，而那正是變更 1 與變更 3
    修掉的東西。而且有效的文件不會累積成 backlog——它一旦在抓取時有效，當輪就會
    被更新並離開這兩條查詢，不會留在那裡等著被擠掉。要湊出「文件有效、卻被一大堆
    持續失敗的影片擠出名額」這個組合，實務上不會發生。選降序而非升序，是因為萬一
    真的發生，被排擠的會是本來就卡死的舊文件，而不是最需要即時收集的新直播。
    rotating cursor 或 retry backoff 都需要新增欄位或外部狀態，與上一條限制拒絕
    失敗計數的理由相同。
