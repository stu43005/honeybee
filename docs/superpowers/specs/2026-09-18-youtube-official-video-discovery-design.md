...
1. 任何訂閱頻道發布的新影片（含上傳影片、shorts、排程直播），即使 pubsub 漏送，也能在**一小時內**被發現。這是盡力而為的覆蓋而非保證——成立的前提列在「Non-goals / Accepted limitations」。
...
### 覆蓋是盡力而為，前提是發布速率不超過窗口

**關切**：兩次成功輪詢之間，若一個頻道發布的影片數超過窗口大小，最舊的那幾支會掉出窗口。pubsub 若同時漏掉它們，就再也沒有機制會發現——這與上面那條「不回補歷史」不同，掉的是上線之後才發布的影片。
...
- 每個輪詢間隔內，單一頻道的公開影片發布數不超過 15 支、會員影片不超過 50 支。
- 服務沒有長時間停機。停機數日後重啟，第一輪只拿得到最新 15 支，停機期間發布的其餘上傳影片與 shorts 會漏掉（直播由 Holodex 的 `update past` 輪詢撿回，它一次抓 100 支）。
- 連續失敗的頻道間隔會被拉長：失敗後時間戳照樣推進、排到隊尾，要再等一整圈，所以上面那個「15 支」的預算會按連續失敗的圈數等比縮小。

這些條件不成立時會漏影片，而系統不會察覺也不會補救。
